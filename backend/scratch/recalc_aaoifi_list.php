...
$symbols = [
    'ABCTRANS', 'ACADEMY', 'AIRTELAFRI', 'ALEX', 'ARADEL', 'AUSTINLAZ', 'BAPLC', 'BERGER', 'BUACEMENT', 'BUAFOODS', 
    'CADBURY', 'CAP', 'CAVERTON', 'CHAMS', 'CHELLARAM', 'CONOIL', 'CUTIX', 'CWG', 'DANGCEM', 'DANGSUGAR', 
    'EKOCORP', 'ENAMELWA', 'ETERNA', 'ETRANZACT', 'EUNISELL', 'FIDSON', 'FTNCOCOA', 'GEREGU', 'HBMNG', 'HONYFLOUR', 
    'IMG', 'JAIZBANK', 'JAPAULGOLD', 'JBERGER', 'JOHNHOLT', 'JULI', 'LEARNAFRCA', 'LEGENDINT', 'MAYBAKER', 'MCNICHOLS', 
    'MECURE', 'MEYER', 'MORISON', 'MTNN', 'MULTITREX', 'MULTIVERSE', 'NASCON', 'NEIMETH', 'NESTLE', 'NNFM', 
    'NREIT', 'OANDO', 'OKOMUOIL', 'OMATEK', 'PHARMDEKO', 'PREMPAINTS', 'PRESCO', 'PZ', 'REDSTAREX', 'RONCHESS', 
    'RTBRISCOE', 'SCOA', 'SEPLAT', 'SKYAVN', 'THOMASWY', 'TIP', 'TOTAL', 'TRANSEXPR', 'TRANSPOWER', 'TRIPPLEG', 
    'UACN', 'UNILEVER', 'UNIONDICON', 'UPDC', 'UPL', 'VITAFOAM', 'NAHCO', 'LOTUSHAL15'
];

$screenings = AaoifiScreening::whereHas('company', function($q) use ($symbols) {
    $q->whereIn('symbol', $symbols);
})->with('company')->get();
...
foreach ($screenings as $aaoifi) {
    ...
    $market_cap = $raw['market_cap'] ?? $c->market_cap ?? 0;
    
    if ($market_cap <= 0) {
        echo "SKIPPED: {$c->symbol} (Market Cap is 0 or null)\n";
        continue;
    }
    ...
    if ($c->symbol === 'JAIZBANK' || str_contains(strtoupper($c->name), 'ISLAMIC') || $c->symbol === 'LOTUSHAL15') {
        $debt_pass = 'pass';
        $cash_pass = 'pass';
        $inc_pass = 'pass';
    }
