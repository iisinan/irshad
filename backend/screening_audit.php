...
foreach ($stocks as $s) {
    ...
    $issues = [];
    if ($businessFail) {
        $issues[] = "Business: FAIL (keyword: '$matchedKeyword' in sector/name/type)";
    }
    if ($debtStatus === 'fail') {
        $issues[] = 'Debt: FAIL ('.round($debtRatio, 2).'% > 30%)';
    }
    if ($debtStatus === 'insufficient_data') {
        $issues[] = 'Debt: MISSING (no market cap)';
    }
    if ($cashStatus === 'fail') {
        $issues[] = 'Cash: FAIL ('.round($cashRatio, 2).'% > 30%)';
    }
    if ($cashStatus === 'insufficient_data') {
        $issues[] = 'Cash: MISSING (no market cap)';
    }
    if ($interestFail) {
        $issues[] = 'Interest Income: FAIL ('.round($interestRatio, 2).'% > 5%)';
    }
    if ($receivablesStatus === 'fail') {
        $issues[] = 'Receivables: FAIL ('.round($receivablesRatio, 2).'% > 45%)';
    }
