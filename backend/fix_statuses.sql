...
SET status = 'halal', reason = 'Manual override applied based on the July 2026 NGX Shariah Excel report.', verified_by_scholar = true, confidence_score = 100, last_updated = NOW(), updated_at = NOW()
WHERE company_id IN (
...
UPDATE stock_statuses
SET status = 'doubtful', reason = 'Manual override applied based on the July 2026 NGX Shariah Excel report.', verified_by_scholar = true, confidence_score = 100, last_updated = NOW(), updated_at = NOW()
WHERE company_id IN (
    SELECT id FROM companies WHERE symbol IN ('AFROMEDIA', 'CMFC', 'DELUXEPROP', 'UHOMREIT', 'ZICHIS')
);

UPDATE stock_statuses
SET status = 'non-halal', reason = 'Manual override applied based on the July 2026 NGX Shariah Excel report.', verified_by_scholar = true, confidence_score = 100, last_updated = NOW(), updated_at = NOW()
WHERE company_id IN (
