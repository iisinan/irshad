...
class PerplexityClient:
    ...
    async def fetch_comprehensive_data(self, company_name: str, financial_year: int) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            f"CRITICAL RULES - NO HALLUCINATION ALLOWED:\n"
            f"1. Search the web for their AUDITED FULL YEAR {financial_year} (FY {financial_year}) financial results.\n"
            f"2. DO NOT use Q1 {financial_year + 1} data, and DO NOT guess or estimate figures. If you cannot find the exact audited {financial_year} figure from a reliable source (like NGX Doclib, Nairametrics, or Company IR), return exactly 0 for that field.\n"
            f"3. Return the figures as RAW numbers printed in the document/news (do NOT pre-scale them). If the news says '5.2 billion NGN', return 5.2 and set unit_multiplier to 1000000000.\n"
            f"4. MARKET CAP: Do not extract Market Cap from old PDF reports. Only use the CURRENT Market Cap from reliable financial websites.\n"
            f"5. BANK DEBT: If this is a bank, total_debt = only interest-bearing borrowings (bonds, loans). Do NOT include customer deposits in total_debt.\n"
            f"6. Provide business activity details and a Halal screening verdict (Compliant, Non-Compliant, or Questionable based on AAOIFI Shariah standards regarding their business lines, e.g., alcohol, gambling, conventional finance).\n"
            f"7. Provide the latest news regarding the stock.\n"
            f"8. SOURCE URLs: You MUST provide the URLs where you found this information in the 'source_urls' object.\n"
            f"9. You MUST return ONLY a valid JSON object matching the exact schema below. Do not include markdown formatting like ```json or any conversational text. Just the raw JSON.\n\n"
            f"SCHEMA:\n"
            f"{{\n"
            f"  \"financials\": {{\n"
