...
class BasketsErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ padding: '20px', background: 'darkred', color: 'var(--bg)', borderRadius: '12px', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
          <h3>Crash inside BasketsTab:</h3>
          <p>{String(this.state.error)}</p>
          <pre style={{ fontSize: '0.7rem' }}>{this.state.error?.stack}</pre>
        </div>
      );
    }
...
function BasketsTabContent() {
  ...
  if (selectedBasket) {
    return (
      ...
      <div className="animate-fade-in stagger-1" style={{ background: 'var(--bg)', borderRadius:'24px', padding:'32px', boxShadow:'var(--shadow-sm)', border:'1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              <h2 style={{ fontSize: '1.23rem', fontWeight: 900, color: 'var(--text-dark)' }}>{selectedBasket.name}</h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.84rem', marginTop:'4px' }}>{selectedBasket.description}</p>
            </div>
          </div>
        ...
        {loadingDetails ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 0', gap: '16px' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.79rem' }}>Loading stocks in basket...</p>
          </div>
        ) : (
            ...
            {basketDetails?.stocks?.map((stock, i) => (
              <div 
                ...
                style={{ 
                  ...
                  background: 'var(--bg)', borderRadius:'16px', border:'1px solid var(--border)', cursor:'pointer',
                  transition:'all 0.2s', animationDelay: `${i * 0.05}s`
                }}
                className="roll-in-anim hover-lift"
              >
                <div style={{ display:'flex', alignItems:'center', gap:'16px' }}>
                  <div style={{ width:'40px', height:'40px', background:'var(--bg-section)', borderRadius:'12px', display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text-muted)', fontWeight:800, fontSize: '0.7rem' }}>
                    {stock.symbol.substring(0, 2)}
                  </div>
                  <div>
                    <h4 style={{ fontSize: '0.88rem', fontWeight:800, color:'var(--text-dark)' }}>{stock.symbol}</h4>
                    <span style={{ fontSize: '0.7rem', color:'var(--text-muted)' }}>{stock.name}</span>
                  </div>
                </div>
                <div style={{ display:'flex', alignItems:'center', gap:'16px' }}>
                  <div style={{ textAlign:'right' }}>
                    <div style={{ fontSize: '0.88rem', fontWeight:800, color:'var(--text-dark)' }}>₦{Number(stock.price || 0).toFixed(2)}</div>
                    <div style={{ fontSize: '0.7rem', fontWeight:700, color: (stock.change || 0) >= 0 ? 'var(--halal)' : 'var(--non-halal)', display:'flex', alignItems:'center', gap:'4px', justifyContent:'flex-end' }}>
        ...
        {/* Invest Modal */}
        {showInvestModal && createPortal(
          <div className="modal-overlay animate-fade-in" style={{ position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(13, 27, 42, 0.4)', backdropFilter:'blur(4px)', zIndex:100000, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
            <div className="modal-content roll-in-anim" style={{ background: 'var(--bg)', borderRadius:'24px', width:'100%', maxWidth:'450px', boxShadow:'var(--shadow-lg)', overflow:'hidden', display:'flex', flexDirection:'column' }}>
              <div style={{ padding:'24px 32px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                <h2 style={{ fontSize: '1.06rem', fontWeight:800, color:'var(--text-dark)' }}>Invest in {selectedBasket.name}</h2>
              ...
              <form onSubmit={handleInvestSubmit} style={{ padding:'32px', display:'flex', flexDirection:'column', gap:'24px' }}>
                <div>
                  <label style={{ display:'block', fontSize: '0.75rem', fontWeight:700, color:'var(--text-dark)', marginBottom:'8px' }}>Amount to Invest (₦)</label>
                  <input 
                    type="number"
                    min="1000"
                    step="100"
                    ...
                    style={{ width:'100%', padding:'16px', borderRadius:'12px', border:'1px solid var(--border)', fontSize: '0.97rem', background:'var(--bg-section)', fontWeight:700 }}
                  />
                  {investAmount && !isNaN(investAmount) && basketDetails?.stocks && basketDetails.stocks.length > 0 && (
                    <div style={{ marginTop:'12px', padding:'12px', background:'var(--primary-50)', borderRadius:'8px', fontSize: '0.75rem', color:'var(--primary)' }}>
                ...
                <button 
                  ...
                  style={{ background:'var(--primary)', color:'var(--bg)', border:'none', borderRadius:'12px', padding:'16px', fontSize: '0.88rem', fontWeight:800, cursor: (investing || !investAmount || parseFloat(investAmount) < 1000) ? 'not-allowed' : 'pointer', opacity: (investing || !investAmount || parseFloat(investAmount) < 1000) ? 0.5 : 1 }}
                >
    ...
    <div className="animate-fade-in stagger-1" style={{ background: 'var(--bg)', borderRadius:'24px', padding:'0', boxShadow:'var(--shadow-sm)', border:'1px solid var(--border)', overflow:'hidden' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0D1B2A 0%, #0F5257 65%, #0B6B71 100%)', padding: '32px', position: 'relative', overflow: 'hidden', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', position: 'relative', zIndex: 1 }}>
          ...
          <div>
            <h2 style={{ fontSize: '1.23rem', fontWeight: 800, color: 'white', letterSpacing: '-0.5px' }}>Thematic Baskets</h2>
            <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.79rem', marginTop: '4px' }}>Curated portfolios based on themes</p>
          </div>
        </div>
        <button 
      ...
      <div style={{ padding: '32px' }}>
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 0', gap: '16px' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.79rem' }}>Loading baskets...</p>
          </div>
        ) : baskets.length === 0 ? (
            ...
            {baskets.map((basket, i) => (
              <div 
                ...
                style={{ 
                  ...
                  background: 'var(--bg)', borderRadius: '20px', padding: '24px', border: '1px solid var(--border)',
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                  ...
                  {basket.user_id && (
                    <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                      <span style={{ fontSize: '0.62rem', fontWeight:800, color:'var(--gold)', background:'rgba(201,168,76,0.1)', padding:'4px 8px', borderRadius:'6px' }}>CUSTOM</span>
                ...
                <h3 style={{ fontSize: '0.97rem', fontWeight: 800, color: 'var(--text-dark)', marginBottom: '8px' }}>{basket.name}</h3>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.79rem', lineHeight: 1.5, flex: 1 }}>{basket.description || 'No description provided.'}</p>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '20px', paddingTop: '16px', borderTop: '1px dashed var(--border)' }}>
                  <span style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--primary)', background: 'var(--primary-50)', padding: '4px 10px', borderRadius: '8px' }}>
      ...
      {showCreateModal && createPortal(
        <div className="modal-overlay animate-fade-in" style={{ position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(13, 27, 42, 0.6)', backdropFilter:'blur(8px)', zIndex:100000, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
          <div className="modal-content roll-in-anim" style={{ background: 'var(--bg)', borderRadius:'24px', width:'100%', maxWidth:'600px', boxShadow:'0 24px 64px rgba(0,0,0,0.2)', overflow:'hidden', display:'flex', flexDirection:'column', maxHeight:'80vh' }}>
            <div style={{ padding:'24px 32px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between', background: 'linear-gradient(to right, var(--bg-section), white)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <h2 style={{ fontSize: '1.1rem', fontWeight:800, color:'var(--text-dark)', margin: 0 }}>Create Custom Basket</h2>
              </div>
            ...
            <form onSubmit={handleCreateBasket} style={{ padding:'32px', overflowY:'auto', display:'flex', flexDirection:'column', gap:'24px' }}>
              <div>
                <label style={{ display:'block', fontSize: '0.79rem', fontWeight:700, color:'var(--text-dark)', marginBottom:'8px' }}>Basket Name</label>
                <input 
                  required
                  value={newBasket.name}
                  onChange={e => setNewBasket({...newBasket, name: e.target.value})}
                  placeholder="e.g. My Dividend Kings"
                  style={{ width:'100%', padding:'14px 16px', borderRadius:'12px', border:'2px solid var(--border)', fontSize: '0.88rem', background:'var(--bg-section)', transition: 'border-color 0.2s', outline: 'none' }}
              ...
              <div>
                <label style={{ display:'block', fontSize: '0.75rem', fontWeight:700, color:'var(--text-dark)', marginBottom:'8px' }}>Description (Optional)</label>
                <textarea 
                  ...
                  rows={2}
                  style={{ width:'100%', padding:'12px 16px', borderRadius:'12px', border:'1px solid var(--border)', fontSize: '0.88rem', background:'var(--bg-section)', resize:'none' }}
                />
              </div>

              <div>
                <label style={{ display:'block', fontSize: '0.75rem', fontWeight:700, color:'var(--text-dark)', marginBottom:'8px' }}>
                  Select Stocks ({newBasket.symbols.length} selected)
                </label>
                <div style={{ border:'1px solid var(--border)', borderRadius:'12px', overflow:'hidden', maxHeight:'200px', overflowY:'auto', background: 'var(--bg)' }}>
                  {allStocks.filter(s => ['halal', 'compliant'].includes(getStatus(s))).map(stock => {
                    ...
                    return (
                      <div 
                        ...
                        style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', borderBottom:'1px solid var(--bg-section)', cursor:'pointer', background: isSelected ? 'var(--primary-50)' : 'var(--bg)', transition:'background 0.2s' }}
                      >
                        <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                          <div style={{ width:'20px', height:'20px', borderRadius:'4px', border: isSelected ? 'none' : '1px solid var(--text-light)', background: isSelected ? 'var(--primary)' : 'transparent', display:'flex', alignItems:'center', justifyContent:'center', color:'var(--bg)' }}>
              ...
              <button 
                ...
                style={{ background:'var(--primary)', color:'var(--bg)', border:'none', borderRadius:'12px', padding:'16px', fontSize: '0.88rem', fontWeight:800, cursor: (saving || !newBasket.name || newBasket.symbols.length === 0) ? 'not-allowed' : 'pointer', opacity: (saving || !newBasket.name || newBasket.symbols.length === 0) ? 0.5 : 1, marginTop:'10px', transition: 'all 0.2s cubic-bezier(0.16,1,0.3,1)' }}
      ...
      {showEditModal && createPortal(
        <div className="modal-overlay animate-fade-in" style={{ position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(13, 27, 42, 0.4)', backdropFilter:'blur(4px)', zIndex:100000, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
          <div className="modal-content roll-in-anim" style={{ background: 'var(--bg)', borderRadius:'24px', width:'100%', maxWidth:'600px', boxShadow:'var(--shadow-lg)', overflow:'hidden', display:'flex', flexDirection:'column', maxHeight:'80vh' }}>
            <div style={{ padding:'24px 32px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <h2 style={{ fontSize: '1.06rem', fontWeight:800, color:'var(--text-dark)' }}>Edit Custom Basket</h2>
            ...
            <form onSubmit={handleEditBasket} style={{ padding:'32px', overflowY:'auto', display:'flex', flexDirection:'column', gap:'20px' }}>
              <div>
                <label style={{ display:'block', fontSize: '0.75rem', fontWeight:700, color:'var(--text-dark)', marginBottom:'8px' }}>Basket Name</label>
                <input 
                  required
                  value={editingBasket.name}
                  onChange={e => setEditingBasket({...editingBasket, name: e.target.value})}
                  placeholder="e.g. My Dividend Kings"
                  style={{ width:'100%', padding:'12px 16px', borderRadius:'12px', border:'1px solid var(--border)', fontSize: '0.88rem', background:'var(--bg-section)' }}
                />
              </div>
              ...
              <button 
                ...
                style={{ background:'var(--primary)', color:'var(--bg)', border:'none', borderRadius:'12px', padding:'14px', fontSize: '0.88rem', fontWeight:800, cursor: (saving || !editingBasket.name || editingBasket.symbols.length === 0) ? 'not-allowed' : 'pointer', opacity: (saving || !editingBasket.name || editingBasket.symbols.length === 0) ? 0.5 : 1, marginTop:'10px' }}
              >
