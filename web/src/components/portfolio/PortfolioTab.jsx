...
function HoldingRow({ holding, onDelete, onEdit, hasBeenPurified }) {
  ...
  const actionBtns = (
    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }} onClick={e => e.stopPropagation()}>
      ...
      <button onClick={() => onEdit(holding)} style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-muted)', padding: '5px', borderRadius: '6px', display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
        <Edit2 size={11}/>
      </button>
      <button onClick={() => onDelete(holding.id)} style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--non-compliant)', padding: '5px', borderRadius: '6px', display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
        <Trash2 size={11}/>
      </button>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      {/* ── Grace Period Banner ── */}
      {gracePeriodEndsAt && (
        <div style={{ padding: '5px 12px', background: 'rgba(239,68,68,0.05)', borderTop: '1px solid rgba(239,68,68,0.1)', display: 'flex', alignItems: 'center', gap: '5px' }}>
          <AlertTriangle size={10} color="#dc2626"/>
          <span style={{ fontSize: '0.58rem', color: '#dc2626', fontWeight: 600 }}>
