        ...
        <div className="animate-fade-in" style={{ position: 'fixed', inset: 0, background: 'rgba(15,82,87,0.4)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100000, padding: '24px' }}>
          <div ref={modalRef} style={{ background: 'var(--bg)', borderRadius: '28px', width: '100%', maxWidth: '720px', boxShadow: '0 32px 80px rgba(0,0,0,0.15), 0 0 0 1px rgba(255,255,255,0.5) inset', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '90vh', animation: 'slideUpFade 0.4s cubic-bezier(0.16, 1, 0.3, 1)' }}>
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--bg-section)' }}>
              <button onClick={() => setTab('manual')} style={{ flex: 1, padding: '16px', background: 'none', border: 'none', fontSize: '0.84rem', fontWeight: 800, color: tab === 'manual' ? 'var(--primary)' : 'var(--text-muted)', borderBottom: tab === 'manual' ? '2px solid var(--primary)' : '2px solid transparent', cursor: 'pointer', transition: 'all 0.2s' }}>Manual Entry</button>
              <button onClick={() => setTab('import')} style={{ flex: 1, padding: '16px', background: 'none', border: 'none', fontSize: '0.84rem', fontWeight: 800, color: tab === 'import' ? 'var(--primary)' : 'var(--text-muted)', borderBottom: tab === 'import' ? '2px solid var(--primary)' : '2px solid transparent', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}><FileText size={14} /> Import Statement</button>
              <button onClick={() => setTab('broker')} style={{ flex: 1, padding: '16px', background: 'none', border: 'none', fontSize: '0.84rem', fontWeight: 800, color: tab === 'broker' ? 'var(--primary)' : 'var(--text-muted)', borderBottom: tab === 'broker' ? '2px solid var(--primary)' : '2px solid transparent', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}><Lock size={14} /> Link Broker</button>
            </div>
              ...
              <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', flex: 1 }}>
                  ...
                  {rows.map((row, index) => (
                    <div key={row.id} style={{ background: 'var(--bg)', padding: '20px', borderRadius: '16px', marginBottom: '20px', border: '1px solid var(--border)', boxShadow: '0 2px 8px rgba(0,0,0,0.02)', position: 'relative' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                        <div style={{ fontSize: '0.7rem', fontWeight: 800, color: 'var(--primary)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Holding #{index + 1}</div>
                        {rows.length > 1 && (
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1.2fr', gap: '12px', alignItems: 'flex-start' }}>
                        ...
                        <div>
                          ...
                          <div style={{ position: 'relative' }}>
                            ...
                            <input value={row.sym} onChange={e => handleRowChange(row.id, 'sym', e.target.value)} onFocus={() => { if (row.sym) setActiveRowId(row.id); }} onBlur={() => setTimeout(() => setActiveRowId(null), 200)} placeholder="Search stock..." style={{ width: '100%', padding: '14px 14px 14px 40px', borderRadius: '12px', border: '2px solid var(--border)', fontSize: '0.88rem', fontWeight: 700, color: 'var(--text-dark)', textTransform: 'uppercase', outline: 'none', transition: 'all 0.2s', background: 'var(--bg-section)' }} onFocusCapture={e => { e.target.style.borderColor = 'var(--primary)'; e.target.style.background = 'var(--bg)'; }} onBlurCapture={e => { e.target.style.borderColor = 'var(--border)'; e.target.style.background = 'var(--bg-section)'; }} />
                        ...
                        {/* Date — REQUIRED */}
                        <div>
                          <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-dark)', marginBottom: '8px' }}>
                            Purchase Date
                          ...
                          <input
                            ...
                            style={{ width: '100%', padding: '14px', borderRadius: '12px', border: `2px solid ${row.date ? 'var(--border)' : 'rgba(239,68,68,0.3)'}`, fontSize: '0.88rem', fontWeight: 700, outline: 'none', background: row.date ? 'var(--bg-section)' : 'rgba(239,68,68,0.04)', transition: 'all 0.2s', color: row.date ? 'var(--text-dark)' : 'var(--text-muted)' }}
                            onFocus={e => { e.target.style.borderColor = 'var(--primary)'; e.target.style.background = 'var(--bg)'; }}
                            onBlur={e => { e.target.style.borderColor = row.date ? 'var(--border)' : 'rgba(239,68,68,0.3)'; e.target.style.background = row.date ? 'var(--bg-section)' : 'rgba(239,68,68,0.04)'; }}
                          />
                          {!row.date && (
                            <div style={{ fontSize: '0.62rem', color: '#ef4444', marginTop: '5px', fontWeight: 600 }}>Required for Zakat calculation</div>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <div style={{ padding: '24px 28px', borderTop: '1px solid var(--border)', background: 'var(--bg)', zIndex: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                    ...
                    <span style={{ fontSize: '1.23rem', fontWeight: 800, color: 'var(--primary)' }}>₦{totalCost.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
