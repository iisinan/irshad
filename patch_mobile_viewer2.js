...
const regex2 = /if \(userPerf\.isNotEmpty\) \.\.\.\[.*?\]\,\n\n                      Text\('Market Top Gainers'.*?\],/s;

// Since it's easier to just match from "if (userPerf.isNotEmpty)" down to the end of the children array

let parts = file.split("if (userPerf.isNotEmpty) ...[");
if (parts.length > 1) {
    let before = parts[0];
    let afterSplit = parts[1].split("                    ],\n                  ),\n                ),\n              ),");
    
    if (afterSplit.length > 1) {
        let replacement2 = `if (complianceChanges.isNotEmpty) ...[
        ...
        // Piece it together
        file = before + replacement2 + afterSplit.slice(1).join("                    ],\n                  ),\n                ),\n              ),");
        
        fs.writeFileSync('mobile/lib/features/portfolio/ui/tabs/updates/updates_inbox_tab.dart', file);
        console.log("Patched successfully!");
    } else {
        console.log("Could not find end split.");
    }
} else {
    console.log("Could not find userPerf split.");
}
