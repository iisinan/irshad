...
"""
    ...
    # We need to replace specific line ranges!
    # _buildFinancialHighlights and _buildAaoifiBreakdown: 1210 to 1641
    start_idx_aaoifi = 1210 - 1
    end_idx_aaoifi = 1641 - 1
    
    # We will replace lines 1210-1641 with new_aaoifi
    # But wait, let's also find the exact line numbers for _buildStatusHeader (596 to 724)
    start_idx_header = 596 - 1
    end_idx_header = 724 - 1
    
    # And we also need to change the 'build' block that calls them.
    # Lines 377 to 404 contains the Shariah Justification, AAOIFI Breakdown, and Financial Highlights
    start_idx_build = 377 - 1
    end_idx_build = 404 - 1
...
"""
    ...
    # Do replacements from bottom to top so line numbers don't shift for earlier blocks
