...
def main():
    ...
    with open('header.dart', 'r') as f:
        old_header = f.read()

    with open('aaoifi.dart', 'r') as f:
        old_aaoifi = f.read()
        
    with open('financials.dart', 'r') as f:
        old_financials = f.read()
        
    with open('calling.dart', 'r') as f:
        old_calling = f.read()
...
"""
    ...
    if old_calling not in content:
        print("Calling block not found")
        sys.exit(1)
    content = content.replace(old_calling, new_calling)
    
    if old_aaoifi not in content:
        print("Aaoifi block not found")
        sys.exit(1)
    content = content.replace(old_aaoifi, new_aaoifi)
    
    if old_financials in content:
        content = content.replace(old_financials, "")

    with open('lib/features/stocks/ui/stock_detail_screen.dart', 'w') as f:
        f.write(content)
        
    print("Successfully replaced strings")
