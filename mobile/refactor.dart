...
void main() {
  var file = File('lib/features/stocks/ui/stock_detail_screen.dart');
  var content = file.readAsStringSync();
  
  // 1. Remove Shariah Justification block in build()
  content = content.replaceFirst(RegExp(r'// Shariah Justification Summary.*?(?=(// About Company))', dotAll: true), '');
  
  // 2. Remove Financial Highlights call in build()
  content = content.replaceFirst(RegExp(r'// Financial Highlights.*?const SizedBox\(height: 32\),\n\s*\]\,\n', dotAll: true), '');
  
  // 3. Rename AAOIFI call to Compliance Dashboard
  ...
  // 4. Move Price Chart into _buildStatusHeader (remove from build)
  content = content.replaceFirst(RegExp(r'                  // Price Chart\n                  if \(\(_currentStock\[\'daily_prices\'\] \?\? \[\]\)\.isNotEmpty &&\n                      \(\(_currentStock\[\'daily_prices\'\] as List\)\.any\(\(p\) => \(double\.tryParse\(p\[\'price\'\]\?\.toString\(\) \?\? \'0\'\) \?\? 0\) > 0\)\)\) \.\.\.\[\n                    _buildPriceChart\(\),\n                    const SizedBox\(height: 32\),\n                  \],\n'), '');
  
  // 5. Replace _buildStatusHeader
  content = content.replaceFirst(RegExp(r'  Widget _buildStatusHeader\(Color color, Color bg, String label, \{bool purificationRequired = false, double percent = 0\.0, bool scholarVerified = false\}\) \{.*?    \);\n  }', dotAll: true), '''
  ...
  // 6. Replace _buildAaoifiBreakdown and _buildFinancialHighlights
  content = content.replaceFirst(RegExp(r'  Widget _buildFinancialHighlights\(\) \{.*?  Widget _buildAiAssistantButton\(\) \{', dotAll: true), '''
